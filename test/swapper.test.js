...
contract("Swapper", ([user, feeRecipient]) => {
  ...
  it("Should swap 2 tokens using best dex", async function () {
    ...
    if (data1.protocols[0][0][0].name === "UNISWAP_V2") {
      const pool = await factory.getPair(WETH_ADDRESS, DAI_ADDRESS);
      // swap1 = [DAI_ADDRESS, pool, 3500, 0]
      swap1 = { token: DAI_ADDRESS, pool, distribution: 3500, dex: 0 }
    }
    else {
      const pools = await balancer.getBestPoolsWithLimit(WETH_ADDRESS, DAI_ADDRESS, 1);
      // swap1 = [DAI_ADDRESS, pools[0], 3500, 1]
      swap2 = { token: DAI_ADDRESS, pool: pools[0], distribution: 3500, dex: 1 }
    }

    if (data2.protocols[0][0][0].name === "UNISWAP_V2") {
      const pool = await factory.getPair(WETH_ADDRESS, LINK_ADDRESS);
      // swap1 = [LINK_ADDRESS, pool, 6500, 0]
      swap1 = { token: LINK_ADDRESS, pool, distribution: 6500, dex: 0 }
    }
    else {
      const pools = await balancer.getBestPoolsWithLimit(WETH_ADDRESS, LINK_ADDRESS, 1);
      // swap1 = [LINK_ADDRESS, pools[0], 6500, 1]
      swap2 = { token: LINK_ADDRESS, pool: pools[0], distribution: 6500, dex: 1 }
    }

    const intialFeeRecipientBalance = await web3.eth.getBalance(feeRecipient);

    const tx = await swapper.swap([swap1, swap2], {
      value: toWei(1),
    });
  ...
  it("Should swap 3 tokens using best dex", async function () {
    ...
    for (let i = 0; i < TOKENS.length; i++) {
      ...
      if (data.protocols[0][0][0].name === "UNISWAP_V2") {
        ...
        const pools = await balancer.getBestPoolsWithLimit(WETH_ADDRESS, token, 1);
